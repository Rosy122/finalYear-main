...
class PainterProfilePage extends StatefulWidget {
  final String providerId;

  const PainterProfilePage({Key? key, required this.providerId})
      : super(key: key);

  @override
  _PainterProfilePageState createState() => _PainterProfilePageState();
}

class _PainterProfilePageState extends State<PainterProfilePage> {
  ...
  Future<void> fetchPainterData() async {
    try {
      ...
      if (doc.exists) {
        setState(() {
          painterData = doc.data() as Map<String, dynamic>;
          isLoading = false;
        });
      } else {
        setState(() {
  ...
  Future<void> incrementLikes() async {
    try {
      ...
      setState(() {
        painterData?['likes'] = (painterData?['likes'] ?? 0) + 1;
      });
  ...
  Widget build(BuildContext context) {
    ...
    final services = painterData?['services'] ?? [];
