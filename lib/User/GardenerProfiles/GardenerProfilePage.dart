...
class GardenerProfilePage extends StatefulWidget {
  ...
  const GardenerProfilePage({Key? key, required this.providerId})
      : super(key: key);

  @override
  _GardenerProfilePageState createState() => _GardenerProfilePageState();
}

class _GardenerProfilePageState extends State<GardenerProfilePage> {
  Map<String, dynamic>? gardenerData;
  bool isLoading = true;
  ...
  Future<void> fetchGardenerData() async {
    try {
      ...
      if (doc.exists) {
        setState(() {
          gardenerData = doc.data() as Map<String, dynamic>;
          isLoading = false;
        });
      } else {
        setState(() {
  ...
  Future<void> incrementLikes() async {
    try {
      ...
      setState(() {
        gardenerData?['likes'] = (gardenerData?['likes'] ?? 0) + 1;
      });
  ...
  Widget build(BuildContext context) {
    ...
    final services = gardenerData?['services'] ?? [];
