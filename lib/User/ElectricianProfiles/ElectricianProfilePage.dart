...
class ElectricianProfilePage extends StatefulWidget {
  ...
  const ElectricianProfilePage({Key? key, required this.providerId})
      : super(key: key);
...
class _ElectricianProfilePageState extends State<ElectricianProfilePage> {
  Map<String, dynamic>? electricianData;
  bool isLoading = true;
  ...
  Future<void> fetchElectricianData() async {
    try {
      ...
      if (doc.exists) {
        setState(() {
          electricianData = doc.data() as Map<String, dynamic>;
          isLoading = false;
        });
      } else {
        setState(() {
  ...
  Future<void> incrementLikes() async {
    try {
      ...
      setState(() {
        electricianData?['likes'] = (electricianData?['likes'] ?? 0) + 1;
      });
  ...
  Widget build(BuildContext context) {
    ...
    final experience = electricianData?['years_of_experience'] ?? 0;
    final likes = electricianData?['likes'] ?? 0;
    final profileImage = electricianData?['profileImage'] ?? '';
    final services = electricianData?['services'] ?? [];
