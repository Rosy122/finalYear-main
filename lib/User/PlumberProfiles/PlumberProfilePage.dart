...
class PlumberProfilePage extends StatefulWidget {
  ...
  const PlumberProfilePage({Key? key, required this.providerId})
      : super(key: key);
...
class _PlumberProfilePageState extends State<PlumberProfilePage> {
  Map<String, dynamic>? plumberData;
  bool isLoading = true;
  ...
  Future<void> fetchPlumberData() async {
    try {
      ...
      if (doc.exists) {
        setState(() {
          plumberData = doc.data() as Map<String, dynamic>;
          isLoading = false;
        });
      } else {
        setState(() {
  ...
  Future<void> incrementLikes() async {
    try {
      ...
      setState(() {
        plumberData?['likes'] = (plumberData?['likes'] ?? 0) + 1;
      });
  ...
  Widget build(BuildContext context) {
    ...
    final services = plumberData?['services'] ?? [];
