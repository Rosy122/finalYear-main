...
class CarpenterProfilePage extends StatefulWidget {
  final String providerId;

  const CarpenterProfilePage({Key? key, required this.providerId})
      : super(key: key);

  @override
  _CarpenterProfilePageState createState() => _CarpenterProfilePageState();
}

class _CarpenterProfilePageState extends State<CarpenterProfilePage> {
  ...
  Future<void> fetchCarpenterData() async {
    try {
      ...
      if (doc.exists) {
        setState(() {
          carpenterData = doc.data() as Map<String, dynamic>;
          isLoading = false;
        });
      } else {
        setState(() {
  ...
  Future<void> incrementLikes() async {
    try {
      ...
      setState(() {
        carpenterData?['likes'] = (carpenterData?['likes'] ?? 0) + 1;
      });
  ...
  Widget build(BuildContext context) {
    ...
    final experience = carpenterData?['years_of_experience'] ?? 0;
    final likes = carpenterData?['likes'] ?? 0;
    final profileImage = carpenterData?['profileImage'] ?? '';
    final services = carpenterData?['services'] ?? [];
