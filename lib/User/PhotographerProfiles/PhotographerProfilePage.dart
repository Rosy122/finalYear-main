...
class PhotographerProfilePage extends StatefulWidget {
  ...
  const PhotographerProfilePage({Key? key, required this.providerId})
      : super(key: key);
...
class _PhotographerProfilePageState extends State<PhotographerProfilePage> {
  Map<String, dynamic>? photographerData;
  bool isLoading = true;
  ...
  Future<void> fetchPhotographerData() async {
    try {
      ...
      if (doc.exists) {
        setState(() {
          photographerData = doc.data() as Map<String, dynamic>;
          isLoading = false;
        });
      } else {
        setState(() {
  ...
  Future<void> incrementLikes() async {
    try {
      ...
      setState(() {
        photographerData?['likes'] = (photographerData?['likes'] ?? 0) + 1;
      });
  ...
  Widget build(BuildContext context) {
    ...
    final experience = photographerData?['years_of_experience'] ?? 0;
    final likes = photographerData?['likes'] ?? 0;
    final profileImage = photographerData?['profileImage'] ?? '';
    final services = photographerData?['services'] ?? [];
